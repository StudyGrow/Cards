...
export class SendPasswordResetMail implements ISendPasswordResetMail {
  ...
  async send(data: ISendPasswordResetMail.Params): Promise<ISendPasswordResetMail.Result> {
    ...
    const deleteToken = () => {
      ...
      this.editUserRepository.editUser({ userId: userData?.user._id || "", data: resetData });
    }
