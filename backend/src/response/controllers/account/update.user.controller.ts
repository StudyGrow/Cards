...
import { unauthenticated, unauthorized, ok, serverError, badRequest, forbidden } from "../../helpers/http.helper";
  ...
  > implements Controller<any, any>
{
  ...
  async handle(request: T1): Promise<HttpResponse<User>> {
    try {
      const error = this.validation.validate({ ...request.user, ...request.data });
      if (error) {
        return badRequest(error);
      }
      const user = await this.updateAccount.update({ data: request.data, user: { userId: request.user.userId } });
      if (!user) {
        return unauthorized();
      }
