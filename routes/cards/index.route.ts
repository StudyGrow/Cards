...
export default class CardsRoute {
  ...
  @GET()
  @before(
    inject(({ validationFactory }) => validationFactory.getCardsByLecture),
  )
  async getCardsByAbreviation(req, res) {
    const lectureAbreviation = req.query.abrv;
    this.cardsService
      .getCardsFromQuery({
        vorlesung: lectureAbreviation,
      })
      .then((cards) => {
